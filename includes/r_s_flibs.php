 ...
 */
function r_s_admin_tools($options) {
    ...
    foreach ($options as $value) {
        ...
        $post_id = get_the_ID();
        $option_value = "";
        $raw_hierarchy = r_wev_explodeX(array('[', ']'), $value["name"]);
        $hierarchy = array_filter($raw_hierarchy);
        $settings_table=  r_wev_get_proper_value($options[0], "table", "metas");
        foreach ($hierarchy as $i => $level) {
            if (!$i) {
                ...
                $option_value = r_wev_get_proper_value($_SESSION, $level, false);
                if (!$option_value)
                {
                ...
                $option_value = r_wev_get_proper_value($option_value, $level, "");
        }
        ...
        switch ($value['type']) {
            ...
            case 'checkbox' :
                ...
                if (!isset($value['checkboxgroup']) || 'start' == $value['checkboxgroup']) {
                    ...
                    if (!empty($value['title'])) {
                        ...
                        $cb_value=  r_wev_get_proper_value($value, "value", false);
                        if(!$cb_value)
                            $cb_value=  r_wev_get_proper_value($value, "default", 1);
                        ?>
                ...
                case 'image' :
                    $set_btn_label=  r_wev_get_proper_value($value, "set", "Set image");
                    $remove_btn_label=  r_wev_get_proper_value($value, "remove", "Remove image");
                    $lazyload=  r_wev_get_proper_value($value, "lazyload", false);
                    ?>
            ...
            case 'repeatable-fields' :
                if (!is_array($option_value))
                    $option_value = array();
                $value["popup"]=  r_wev_get_proper_value($value, "popup", false);
                ...
                foreach ($option_value as $i => $row) {
                    ...
                    foreach ($value["fields"] as $field) {
                        ...
                        echo r_s_admin_tools(array($field));
                    }
                    ?>
                        <td>
                ...
                $row_tpl= r_wev_get_row_template($value);
...
//                    $o_row_templates=array();
                ...
                $add_label=  r_wev_get_proper_value($value, "add_btn_label", __("Add", "vpc"));
                ?>
                </tbody>
                </table>
...
function r_wev_get_row_template($value) {
...
//       ob_start();
        ...
        $row_tpl.=r_s_admin_tools(array($field_tpl));
 ...
 * @return mixed
 */
function r_wev_get_proper_value($values, $search_key, $default_value = "") {
    if (isset($values[$search_key]))
        $default_value = $values[$search_key];
    return $default_value;
}

function r_wev_explodeX($delimiters, $string) {
 ...
 * @return type
 */
function r_wev_get_media_url($media_id) {
    $attachment = wp_get_attachment_image_src($media_id, "full");
    $attachment_url = $attachment[0];
    return $attachment_url;
}
function r_wev_startWith($haystack, $needle) {
    // search backwards starting from haystack length characters from the end
    return $needle === "" || strrpos($haystack, $needle, -strlen($haystack)) !== FALSE;
}

function r_wev_endsWith($haystack, $needle) {
